...
|--------|--------------|------------------|----------------|--------------|--------|
| MSE (Test Set) | ≤ 0.001 | 0.00007933 | 0.00007933 | 0 | ✓ |
| MAE (Test Set) | ≤ 0.02 | 0.004085 | 0.004085 | 0 | ✓ |
| R² (Test Set) | ≥ 0.95 | 0.998147 | 0.998147 | 0 | ✓ |
...
| Input automat număr de înmatriculare | OCR din imagine (EasyOCR) → candidat plăcuță | UI (desktop/web) | Timp de citire OCR + rata de citire corectă (de completat) |
...
| **Număr total observații finale (N)** | 948 (Train 664 / Val 142 / Test 142) |
| **Număr features** | 3 |
...
| Câmp | Valoare |
|------|---------|
| **Total observații finale (N)** | 948 |
| **Observații originale (M)** | 948 |
...
Datasetul final pentru RN (`data/processed/nn_dataset.csv`) este construit de `src/preprocessing/dataset_builder.py` ca un cross-join între plăcuțe și scenariile de intersecții (în acest repo: 79 plăcuțe × 12 scenarii = 948 observații), iar eticheta `label_risk` este calculată euristic din cele 3 componente normalizate.
...
|------|----------------------------|---------:|---------:|--------:|----------------|------------|
| **Baseline** | 3-16-8-1, LR=0.003, Dropout=0.0 | 0.00007933 | 0.004085 | 0.998147 | ~1.9s | Cel mai bun (dataset simplu, label euristic) |
| Exp 1 | LR: 0.003 → 0.001 | 0.00029860 | 0.011085 | 0.993026 | ~2.0s | Învățare prea lentă → underfitting |
| Exp 2 | LR: 0.003 → 0.01 | 0.00013783 | 0.006077 | 0.996781 | ~2.0s | LR prea mare → convergență mai slabă |
| Exp 3 | Rețea mai mare: 3-32-16-1 | 0.00017760 | 0.007598 | 0.995852 | ~2.5s | Capacitate mai mare nu ajută pe datele curente |
| Exp 4 | Dropout: 0.0 → 0.2 | 0.00027603 | 0.010919 | 0.993553 | ~2.1s | Dropout scade performanța (problemă deja simplă) |
| **FINAL** | Baseline (ales) | **0.00007933** | **0.004085** | **0.998147** | ~1.9s | **Modelul folosit în UI** |

**Justificare alegere model final:**

*[1 paragraf: De ce această configurație? Ce compromisuri ați făcut între MSE/MAE/R², timp și complexitate?]*

Modelul final ales este **Baseline**, deoarece în experimentele rulate este configurația cu **cel mai mic MSE** și **cel mai mare R²**, având totodată timp mic de antrenare. Pe un dataset tabular foarte simplu (3 feature-uri + etichetă euristică), mărirea rețelei sau introducerea dropout nu a adus beneficii.
...
|--------|---------|--------------|--------|
| **MSE (regresie)** | 0.00007933 | ≤ 0.001 | ✓ |
| **MAE (regresie)** | 0.004085 | ≤ 0.02 | ✓ |
| **RMSE (regresie)** | 0.008907 | ≤ 0.05 | ✓ |
| **R² (regresie)** | 0.998147 | ≥ 0.95 | ✓ |

**Îmbunătățire față de Baseline (Etapa 5):**

| Metric | Etapa 5 (Baseline) | Etapa 6 (Optimizat) | Îmbunătățire |
|--------|-------------------|---------------------|--------------|
| MSE (test) | 0.00007933 | 0.00007933 | 0 |
| MAE (test) | 0.004085 | 0.004085 | 0 |
| R² (test) | 0.998147 | 0.998147 | 0 |
...
| Aspect | Observație |
|--------|------------|
| **Observație cheie** | Pentru pragurile curente LOW/MEDIUM/HIGH, erorile de regresie sunt suficient de mici încât predicțiile nu traversează pragurile pe test set. |
...
|---:|--------------|------:|------:|--------:|----------|----------|-----------------|
| 1 | AG444POV @ Piața Universității / Seara | 1.000000 | 0.913606 | 0.086394 | HIGH | HIGH | Saturație Sigmoid aproape de 1.0 (extremele se comprimă) |
| 2 | AR06XAA @ Piața Romană / Prânz | 0.021739 | 0.050887 | 0.029148 | LOW | LOW | La scoruri foarte mici apar erori relative mai mari (abs_err tot mic) |
| 3 | GR76SUR @ Piața Universității / Seara | 0.874359 | 0.857038 | 0.017321 | HIGH | HIGH | Diferențe mici în zona HIGH, fără traversare prag |
| 4 | MS22MUH @ Piața Universității / Seara | 0.871795 | 0.856456 | 0.015339 | HIGH | HIGH | Idem (aproape de plafon) |
| 5 | BT789JOC @ Piața Romană / Prânz | 0.052508 | 0.066620 | 0.014111 | LOW | LOW | Zgomot/variabilitate mică pe zona LOW |
...
├── .venv314/
│
├── PETRE_Horia_632AB_README_Proiect_RN (1).md    # README principal (de redenumit la predare)
├── README.md                                     # Pornire rapidă UI desktop
...
```
Python 3.12+ (proiectul include mai multe venv-uri; poate rula și pe 3.14)
pip >= 21.0
```
...
```powershell
cd "d:\proiectRN\Proiect retele neuronale"
...
| MSE pe test set | ≤ 0.001 | 0.00007933 | ✓ |
| R² pe test set | ≥ 0.95 | 0.998147 | ✓ |
...
2. **Date puține** pentru plăcuțe (79) și intersecții (4) → generalizare limitată.
